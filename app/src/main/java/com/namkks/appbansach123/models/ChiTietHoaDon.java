...
public class ChiTietHoaDon extends HoaDon{
    ...
    public static ArrayList<String> getLoaiInHD(){
        ...
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("SELECT DISTINCT s.Loai FROM `chitiethoadon` cthd, sach s WHERE cthd.id_sach = s.id");
            ResultSet rs = stm.executeQuery();
            while (rs.next()){
    ...
    public static int getSLLoai(String loai){
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("SELECT Sum(SoLuong) FROM `chitiethoadon` cthd, sach s where s.id = cthd.id_sach AND s.Loai = ?");
    ...
    public static int getTongTien(){
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("SELECT SUM(cthd.SoLuong * s.GiaTien) FROM `chitiethoadon` cthd, sach s WHERE cthd.id_sach = s.id");
            ResultSet rs = stm.executeQuery();
            rs.next();
            return rs.getInt(1);
        }catch (SQLException e){
