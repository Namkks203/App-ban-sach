...
public class KhachHang {
    ...
    public static ArrayList<KhachHang> TimKiemKH(String sdt){
        ...
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("SELECT * FROM `khachhang` WHERE SDT = ?");
            stm.setString(1, sdt);
            ResultSet rs = stm.executeQuery();
            rs.next();
            KhachHang kh = new KhachHang();
    ...
    public static boolean XoaKH(int id){
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("DELETE FROM `khachhang` WHERE id = ?");
            stm.setInt(1, id);
            return stm.executeUpdate() > 0 ;
        }catch (SQLException e){
            return false;
    ...
    public boolean SuaKH(){
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("UPDATE `khachhang` SET `HoTen`= ?,`SDT`=?,`Pass`=?,`DiaChi`=? WHERE id = ?");
            stm.setString(1, this.hoTen);
            stm.setString(2, this.sdt);
            stm.setString(3, this.pass);
            stm.setString(4, this.diaChi);
    ...
    public static ArrayList<KhachHang> getAllKH(){
        ...
        try{
            DAO a = new DAO();
            PreparedStatement stm = a.conn.prepareStatement("SELECT * FROM `khachhang` order by id DESC");
            ResultSet rs = stm.executeQuery();
            while (rs.next()){
                KhachHang kh = new KhachHang();
                kh.setId(rs.getInt(1));
